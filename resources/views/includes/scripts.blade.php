...
(function() {
        ...
        .then(data => {
            if (data.success && data.branches && data.branches.length > 0) {
                ...
                data.branches.forEach(branch => {
                    ...
                    const whatsappNumber = branch.whatsapp || branch.phone || '';
                    html += `
                        <div class="branch-card" data-branch-id="${branch.id}" data-branch-whatsapp="${whatsappNumber}">
                            ...
                            ${branch.address ? `<div class="branch-card-info"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg><span>${branch.address}</span></div>` : ''}
                            ${branch.phone ? `<div class="branch-card-info"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg><span>${branch.phone}</span></div>` : ''}
                            <div class="branch-card-actions">
                                ${branch.phone ? `<a href="tel:${branch.phone}" class="branch-action-btn branch-action-call"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M21.384 17.752a2.108 2.108 0 0 1-.522 3.359 7.674 7.674 0 0 1-5.478.642C4.933 20.428 1.48 7.378 4.268 3.384a2.108 2.108 0 0 1 3.359-.522l2.409 2.409a2.108 2.108 0 0 1 .396 2.396l-.923 1.846a.316.316 0 0 0 .063.396c1.429 1.114 3.312 2.997 4.426 4.426a.316.316 0 0 0 .396.063l1.846-.923a2.108 2.108 0 0 1 2.396.396l2.409 2.409z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>{{ __("common.Call") }}</span></a>` : ''}
                                ${hasWhatsapp ? `<button type="button" class="branch-action-btn branch-action-whatsapp" data-branch-whatsapp="${whatsappNumber}" data-order-type="single-product"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg><span>WhatsApp</span></button>` : ''}
                            </div>
                        </div>
                    `;
                });
                ...
                // Attach click listeners to WhatsApp buttons for single product
                document.querySelectorAll(`#${branchListId} .branch-action-whatsapp[data-order-type="single-product"]`).forEach(btn => {
    ...
    window.loadBranchesAndShowModal = function(modalId = 'branchSelectionModal', branchListId = 'branchList') {
        ...
        .then(data => {
            if (data.success && data.branches && data.branches.length > 0) {
                    ...
                    html += `
                        <div class="branch-card" data-branch-id="${branch.id}" data-branch-whatsapp="${whatsappNumber}">
                            ...
                            <div class="branch-card-actions">
                                ...
                                ${hasWhatsapp ? `
                                <button type="button" class="branch-action-btn branch-action-whatsapp" data-branch-whatsapp="${whatsappNumber}" data-order-type="cart">
                ...
                document.querySelectorAll(`#${branchListId} .branch-action-whatsapp`).forEach(btn => {
                    btn.addEventListener('click', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        const branchCard = this.closest('.branch-card');
                        const branchId = branchCard.getAttribute('data-branch-id');
                        const branchWhatsapp = this.getAttribute('data-branch-whatsapp');
    ...
    window.selectBranchAndOpenWhatsApp = function(branchId, branchWhatsapp, modalId = 'branchSelectionModal') {
        ...
        // Update cart counter
        const cartCounters = document.querySelectorAll('.js-cart-items-count');
        ...
        if (typeof toastr !== 'undefined') {
            toastr.success('{{ __("scripts.Sifarişiniz WhatsApp-a göndərildi. Səbətiniz təmizləndi.") }}', '{{ __("scripts.Uğurlu") }}');
        }
    }
