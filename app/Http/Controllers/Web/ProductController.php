...
class ProductController extends Controller
{
    ...
    public function index(string $locale, Request $request): Factory|View
    {
        ...
        $schemaJsonLd = $this->buildIndexSchemaFor($products);

        // Always return full page - JavaScript will parse it
        return view('pages.products.index', compact('products', 'categories', 'brands', 'priceMin', 'priceMax', 'filters', 'shopBanner', 'schemaJsonLd'));
    }

    /**
     * Build structured data schema for products index page
     *
     * @param LengthAwarePaginator $products
     * @return string
     */
    private function buildIndexSchemaFor(LengthAwarePaginator $products): string
    {
        $locale = app()->getLocale();
        $url = route('products.index', ['locale' => $locale]);
        $pageTitle = __('navigation.Products');
        ...
        $webPage = Schema::collectionPage()
            ...
            ->url($url)
            ->inLanguage($locale)
            ->description(__('page.Browse all products available in our store.'));
